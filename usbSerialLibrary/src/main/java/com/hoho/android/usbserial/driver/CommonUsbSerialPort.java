...
public abstract class CommonUsbSerialPort implements UsbSerialPort {
    ...
    // in this method.
    protected abstract boolean initPortSepcific(UsbManager usbManager)
            throws IOException, AccessControlException;

    // Implementors implement their port specific initialization
    // in this method.
    protected abstract boolean initOnlyPortSepcific(UsbDeviceConnection connection)
            throws IOException, AccessControlException;
    ...
    @Override
    public final boolean open(UsbDeviceConnection connection) throws IOException{


        mOpen = initOnlyPortSepcific(connection);
        return mOpen;
    }
    @Override
    public final boolean open(UsbManager usbManager) throws IOException,
            AccessControlException {
        if (mOpen) {
            throw new  IllegalStateException("Port already open");
        }

        mOpen=initPortSepcific(usbManager);
        return mOpen;
    }
